...
/**
 * POST: Add an education record to candidate profile
 */
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info(`API: Adding education record for candidate: ${profile.id}`);

    const education = await candidateService.addEducation(profile.id, body);
    return NextResponse.json({ data: education }, { status: 201 });
  } catch (err: unknown) {
    logger.error("API error in education POST route", err);
