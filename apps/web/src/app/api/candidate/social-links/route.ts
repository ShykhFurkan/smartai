...
/**
 * POST: Add a social link record to candidate profile
 */
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info(`API: Adding social link record for candidate: ${profile.id}`);

    const socialLink = await candidateService.addSocialLink(profile.id, body);
    return NextResponse.json({ data: socialLink }, { status: 201 });
  } catch (err: unknown) {
    logger.error("API error in social link POST route", err);
