...
export async function DELETE(_request: NextRequest, { params }: RouteContext) {
  try {
    ...
    logger.info(`API: Deleting project record: ${projId} for candidate: ${profile.id}`);
    await candidateService.deleteProject(profile.id, projId);

    return NextResponse.json({ success: true, message: "Project record removed successfully" });
  } catch (err: unknown) {
    logger.error("API error in project DELETE route", err);
